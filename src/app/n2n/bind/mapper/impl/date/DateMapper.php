...
class DateMapper extends SingleMapperAdapter {
	...
	protected function createValidators(N2nLocale $n2nLocale): array {
		...
		if ($this->min !== null) {
			$validators[] = Validators::valueClosure(fn($date) => ($date === null || $date->isGreaterThanOrEqualTo($this->min))
					? true
					: ValidationMessages::notEarlierThan(L10nUtils::formatDate($this->min->toDateTimeImmutable(), $n2nLocale)));
		}

		if ($this->max !== null) {
			$validators[] = Validators::valueClosure(fn($date) => ($date === null || $date->isLessThanOrEqualTo($this->max))
					? true
