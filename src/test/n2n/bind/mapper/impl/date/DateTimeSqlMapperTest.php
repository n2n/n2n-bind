...
class DateTimeSqlMapperTest extends TestCase {
	...
	function testAttrs() {
		...
		$result = Bind::attrs($sourceDataMap)->toAttrs($toDataMap)
				->props(['DateTime1', 'DateTime2', 'DateTimeImmutable1', 'Null'], Mappers::dateTimeSql())
				->exec($this->getMockBuilder(MagicContext::class)->getMock());

		$this->assertTrue($result->isValid());

		$this->assertEquals($dateTime1->format(DateUtils::SQL_DATE_TIME_FORMAT), $toDataMap->reqString('DateTime1'));
		$this->assertEquals('2010-01-01 08:09:10', $toDataMap->reqString('DateTime2'));
		$this->assertEquals($dateTimeImmutable1->format(DateUtils::SQL_DATE_TIME_FORMAT), $toDataMap->reqString('DateTimeImmutable1'));
